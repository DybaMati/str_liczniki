...
def test_maj2026_split_checksum_pdf():
    if not PDF.is_file():
        return
    p = parse_energa_pdf_bytes(PDF.read_bytes())
    meters = [
        {"meter_id": "7", "kwh": 132.7, "label": "Tomek"},
        {"meter_id": "8", "kwh": 179.4, "label": "Lonia"},
        {"meter_id": "9", "kwh": 104.3, "label": "Henia"},
    ]
    sp = compute_split(p, meters, {})
    assert sp["checksum"]["all_ok"]
    assert sp["checksum"]["sum_razem_platne"] == 142.01
